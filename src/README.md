...
Second option (you can add the export to your .bashrc):
...
The primal model directly handles the labeled graph encoding the input WVCP instance _without any preliminary reformulation_.
...
We provide below samples of commands for each model and then discuss additional options to extract results from the output. Each command is assumed to be launched from the source code directory `src`.
