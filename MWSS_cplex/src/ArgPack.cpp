...
ArgPack::ArgPack(int argc, char *const argv[])
    : verbose(true), rand_seed(1), target(0), complement(0),
      iterations(INT_MAX), nb_threads(1), time(INT_MAX), p{1, 1, 4, 1} {
  ...
  string usage =
      ...
      " [options] <instance file> <weight file>\n" +
      "Compile time: " + __DATE__ + " " + __TIME__ + "\n" +
      "	-h			: show this help\n" +
      "	-i			: maximum number of iterations [default: " +
      to_string(iterations) + "] \n" +
      "	-s<random seed>		: random seed [default: " +
      to_string(rand_seed) + "]\n" +
      "	-v			: disable verbose mode \n" +
      //	               "	-o<output>      : output solution file
      //[default: " + output_name + "]\n" +
      "	-n<nb_threads>		: number of threads (default 1)\n" +
      "	-t<target>		: stop execution if the target is found\n" +
      "	-T<time>		: stop execution if the execution time exceeds "
      "a certain time [default: " +
      to_string(time) + "]\n" +
      "	-W			: run the algorithm on the graph's "
      "complement\n" +
      "	-p p1:p2:p3:p4		: search exploration/intensification "
      "parameters [default: p1=" +
      to_string(p[0]) + " p2=" + to_string(p[1]) + " p3=" + to_string(p[2]) +
      " p4=" + to_string(p[3]) + "]\n";
  string help = "Use -h for more information\n";

  const char *opt_str = "hs:vt:T:Wp:i:n";
