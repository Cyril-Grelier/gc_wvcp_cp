...
Graph *auxGraphConversion(const Graph *graph) {
  ...
  for (int v1 = 0; v1 < graph->n(); v1++) {
    ...
    for (int j = 0; j < graph->n(); j++) {
      if (incidenceMatrix[v1][j] == 1) {
        output.push_back(j);
      } else if (incidenceMatrix[v1][j] == -1) {
        input.push_back(j);
      }
    }
    ...
    for (int e1 = 0; e1 < (int)output.size() - 1; e1++) {
      for (int e2 = e1 + 1; e2 < (int)output.size(); e2++) {
        ...
        if (incidenceMatrix[v][w] == 0) {
          assert(graph->weight(v1) >= graph->weight(w));
          assert(graph->weight(v1) >= graph->weight(v));
          int map1 = mapMatrix[v1][w];
          int map2 = mapMatrix[v1][v];
    ...
    for (int e1 = 0; e1 < (int)input.size() - 1; e1++) {
      for (int e2 = e1 + 1; e2 < (int)input.size(); e2++) {
        int w = input[e1];
        int v = input[e2];
        assert(graph->weight(w) >= graph->weight(v1));
        assert(graph->weight(v) >= graph->weight(v1));
        int map1 = mapMatrix[w][v1];
        int map2 = mapMatrix[v][v1];
        assert(map1 != -1 && map2 != -1);
        assert(graph->weight(v1) == aux->weight(map1));
        assert(graph->weight(v1) == aux->weight(map2));
        aux->addEdge(map1, map2);
      }
    }
    ...
    for (int e1 = 0; e1 < (int)input.size(); e1++) {
      for (int e2 = 0; e2 < (int)output.size(); e2++) {
        ...
        assert(graph->weight(w) >= graph->weight(v1));
        assert(graph->weight(v1) >= graph->weight(v));
        int map1 = mapMatrix[v1][v];
        int map2 = mapMatrix[w][v1];
        assert(map1 != -1 && map2 != -1);
...
Graph *readInstance(const string &filename1, const string &filename2) {
  ...
  if (sscanf(buffer, "p %*s %d %d", &v1, &v2) == 2) {
    if (v1 < 0 || v2 < 0) {
      input1.close();
      throw InitError(
          "syntax error in line " + std::to_string(linenum) + " of " +
          filename1 +
    ...
    // if (m_count != m) {
    // 	throw InitError("the number of edges announced is not equal to the
    ...
    if (v1 < 0 || v2 < 0) {
      ...
      throw InitError("syntax error in line " + std::to_string(linenum) +
                      " of " + filename1 +
                      ". Matrix dimensions should not be negative.\n");
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    cplex.setParam(IloCplex::Threads, ArgPack::ap().nb_threads);
    cout << "Total weight = " << graph_complement->total_weight() << "\n";
    cplex.solve();

    cout << "Result = "
         << graph_complement->total_weight() - cplex.getObjValue() << endl;
