...
Graph *readInstance (const string &filename1, const string &filename2)
{
	...
	if (sscanf(buffer, "p %*s %d %d", &v1, &v2) == 2) {
		if (v1 < 0 || v2 < 0) {
			input1.close();
			throw InitError("syntax error in line " + std::to_string(linenum) + " of " + filename1 +
		...
		while (!filename2.empty() && input2.getline(buffer, 256)) {
			if (sscanf(buffer, "%d", &v1) == 1) {
				...
				input1.close();
				throw InitError("syntax error in line " + std::to_string(linenum + 1) + " of " + filename2 + "\n.");
			}
...
int main(int argc, char *argv[])
{
	try {
		...
		Graph *graph_complement = readInstance(ArgPack::ap().input_name1, ArgPack::ap().input_name2);
		...
		cout << "Result = " << graph_complement->total_weight() - cplex.getObjValue() << endl;
